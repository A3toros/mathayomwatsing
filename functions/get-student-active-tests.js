...
exports.handler = async function(event, context) {
  ...
  try {
    ...
    console.log('get-student-active-tests called with student_id:', student_id);
    ...
    // Convert student grade format to match test assignment format
    // Student has grade: 'M1', class: '1/15' but tests are assigned with grade: '1', class: '15'
    let assignmentGrade = grade;
    let assignmentClass = className;
    
    if (grade.startsWith('M')) {
        assignmentGrade = grade.substring(1); // Remove 'M' prefix
    }
    
    // Convert class format: '1/15' -> '15', '2/16' -> '16', etc.
    if (className.includes('/')) {
        assignmentClass = className.split('/')[1]; // Get the part after the slash
    }
    
    console.log('Converted to assignment format - grade:', assignmentGrade, 'type:', typeof assignmentGrade, 'class:', assignmentClass, 'type:', typeof assignmentClass);
      ...
      try {
        ...
        if (assignment.test_type === 'multiple_choice') {
          const mcTest = await sql`
            SELECT test_name, num_questions, teacher_id 
