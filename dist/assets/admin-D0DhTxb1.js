import{b as m,a as g,s as u}from"./shared-C67Vj5xU.js";async function I(){if(!await m()){console.error("No valid admin session found in loadAllSubjects, redirecting to login"),g("login-section");return}try{const n=await(await window.tokenManager.makeAuthenticatedRequest("/.netlify/functions/get-all-subjects")).json();n.success&&B(n.subjects)}catch(e){console.error("Error loading subjects:",e)}}async function B(t){if(!await m()){console.error("No valid admin session found in displayAllSubjects, redirecting to login"),g("login-section");return}const n=document.getElementById("allSubjectsContainer");n&&(n.innerHTML=`
    ...
    `)}function L(t,e){if(t.length===0){e.innerHTML="<p>No subjects found.</p>";return}const n=document.createElement("table");n.innerHTML=`
        <thead>
            <tr>
                <th>Subject ID</th>
                <th>Subject</th>
  ...
  `}async function $e(t,e){const n=document.querySelector(`tr:has(button[onclick*="${t}"]) td:nth-child(2)`).textContent;if(confirm(`Are you sure you want to delete this test?

Test: ${n}
Type: ${e.replace("_"," ")}
ID: ${t}

This action cannot be undone.`))try{const s=window.tokenManager.getAccessToken(),o=window.tokenManager.decodeToken(s);if(!o||o.role!=="admin"){alert("Error: Admin session required");return}const a=await window.tokenManager.makeAuthenticatedRequest("/.netlify/functions/delete-test",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({test_type:e,test_id:t})}),c=await a.json();a.ok&&c.success?(alert(`Test deleted successfully: ${c.message}`),v()):alert(`Error deleting test: ${c.error||"Unknown error"}`)}catch(s){console.error("Error deleting test:",s),alert("Error deleting test. Please try again.")}}const S=document.getElementById("debugFunctionsBtn");S&&S.addEventListener("click",showDebugFunctions);const E=document.getElementById("editSubjectsAdminBtn");E&&E.addEventListener("click",showAdminSubjectEditor);const C=document.getElementById("checkAcademicYearBtn");C&&C.addEventListener("click",M);async function Ae(){if(!await m()){console.error("No valid admin session found in loadAdminData, redirecting to login"),g("login-section");return}await A(),await I(),await k(),await $()}export{pe as A,be as B,Ce as C,ke as D,Te as E,V as F,K as G,$e as H,ae as a,ce as b,re as c,le as d,ie as e,de as f,ue as g,se as h,me as i,ge as j,we as k,Ae as l,Se as m,Ee as n,he as o,Y as p,w as q,ye as r,oe as s,ne as t,z as u,J as v,D as w,P as x,i as y,fe as z};
